...
public class JavaHttpClientDemo {
    ...
    public Token requestPostToken(String secretKey, String secretId) {
    ...
    public BankConnection connectToBank (String institutionIdName) {
        CloseableHttpClient client = HttpClients.createDefault();
        ...
        StringEntity params = null;
        try {
            params = new StringEntity("{\"redirect\":\"http://localhost:8080/user/app\",\"institution_id\":\"ALIOR_ALBPPLPW\"}");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        httpPost.setEntity(params);
    ...
    public Transactions requestTransactions (String account) {
        CloseableHttpClient client = HttpClients.createDefault();
        HttpGet httpGet = new HttpGet("https://ob.nordigen.com/api/v2/accounts/892793eb-b3a9-4a5e-8968-c6278b67b89d/transactions/");
        httpGet.addHeader("accept", "application/json");
        httpGet.addHeader("Authorization", "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNjM5OTQ2OTM0LCJqdGkiOiIxOGI5YTFjNjM4M2U0YjQ2ODk1MjhjMjk5MTkwMDJlMCIsImlkIjo1ODE3LCJzZWNyZXRfaWQiOiJlNjlmZGIyMC04ODZjLTRjODgtOGU5Zi01MzJhYTI4MjM0YTEiLCJhbGxvd2VkX2NpZHJzIjpbIjAuMC4wLjAvMCJdfQ.ATvB8kbR2ei4dKd7QuRStEGWiT9zBWeq0elZkSpwX_k");
        ObjectMapper mapper = new ObjectMapper();
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
